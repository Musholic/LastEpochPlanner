...
* There are many files in the `./src/Data` directory that are automatically generated. This is indicated by the header 
...
* Holding `Ctrl` while launching the program will rebuild the mod cache.
  * If this does not work, you can start the program with `REGENERATE_MOD_CACHE="1"` as env variable, e.g. `$env:REGENERATE_MOD_CACHE="1"; & ".\runtime\Last Epoch Planner.exe"`
