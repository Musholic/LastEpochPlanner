...
Below are some common color codes LEP uses:	]]
	...
	for i, damageType in ipairs(DamageTypes) do
		...
		self.controls[damageType:lower()] = new("ButtonControl", { "TOPLEFT", self.controls.normal, "TOPLEFT" }, x, y,
			100, 18, DamageTypesColored[i]:upper(), function () self:SetColor(DamageTypeColors[i]) end)
	end

	self.controls.default = new("ButtonControl", { "TOPLEFT", self.controls.poison, "TOPLEFT" }, 120, 0, 100, 18,
		"^7DEFAULT", function () self:SetColor("^7") end)

	self.controls.edit = new("EditControl", { "TOPLEFT", self.controls.physical, "TOPLEFT" }, 0, 48, 0, 0, "", nil,
		"^%C\t\n", nil, nil, 16, true)
