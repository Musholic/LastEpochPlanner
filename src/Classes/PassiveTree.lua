...
local PassiveTreeClass = newClass("PassiveTree", function (self, treeVersion)
	...
	if self.alternate_ascendancies then
		...
		local alternate_ascendancies_class = { ["name"] = "alternate_ascendancies", ["classes"] = self.alternate_ascendancies }
	...
	for name, data in pairs(self.assets) do
		self:LoadImage(name .. ".png", cdnRoot .. (data[1] or ""), data,
			not name:match("[OL][ri][bn][ie][tC]") and "ASYNC" or nil) --, not name:match("[OL][ri][bn][ie][tC]") and "MIPMAP" or nil)
	end

	-- Load sprite sheets and build sprite map
	self.spriteMap = {}
	self.nodeOverlay = { Normal = { artWidth = 40, }, ClassStart = { artWidth = 80, }, AscendClassStart = { artWidth = 80, } }
