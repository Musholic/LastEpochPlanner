...
local ImportTabClass = newClass("ImportTab", "ControlHost", "Control", function (self, build)
	...
	self.controls.sectionCharImport = new("SectionControl", { "TOPLEFT", self, "TOPLEFT" }, 10, 18, 750, 300,
		"Character Import")
	...
	self.controls.accountNameGoOffline = new("ButtonControl",
		{ "TOPLEFT", self.controls.accountNameHeaderOffline, "BOTTOMLEFT" }, 0, 4, 100, 20, "Start Offline", function ()
			self.isOnlineMode = false
	...
	self.controls.accountName = new("EditControl", { "TOPLEFT", self.controls.accountNameHeader, "BOTTOMLEFT" }, 0, 4,
		200, 20, main.lastAccountName or "", nil, "%c", nil, nil, nil, nil, true)
	...
	self.controls.accountNameGo = new("ButtonControl", { "LEFT", self.controls.accountName, "RIGHT" }, 8, 0, 100, 20,
	...
	self.controls.accountHistory = new("DropDownControl", { "LEFT", self.controls.accountNameGo, "RIGHT" }, 8, 0, 200, 20,
	...
	self.controls.removeAccount = new("ButtonControl", { "LEFT", self.controls.accountHistory, "RIGHT" }, 8, 0, 20, 20,
		"X", function ()
	...
	self.controls.charSelectLeagueLabel = new("LabelControl", { "TOPLEFT", self.controls.charSelectHeader, "BOTTOMLEFT" },
		0, 6, 0, 14, "^7League:")
	self.controls.charSelectLeague = new("DropDownControl", { "LEFT", self.controls.charSelectLeagueLabel, "RIGHT" }, 4,
		0, 150, 18, nil, function (index, value)
			self:BuildCharacterList(value.league)
		end)
	self.controls.charSelect = new("DropDownControl", { "TOPLEFT", self.controls.charSelectHeader, "BOTTOMLEFT" }, 0, 24,
		400, 18)
	self.controls.charSelect.enabled = function ()
		return self.charImportMode == "SELECTCHAR"
	end
	self.controls.charDownload = new("ButtonControl", { "TOPLEFT", self.controls.charSelect, "BOTTOMLEFT" }, 0, 8, 100,
		20, "Download", function ()
	...
	self.controls.charImportHeader = new("LabelControl", { "TOPLEFT", self.controls.charSelect, "BOTTOMLEFT" }, 0, 40,
		200, 16, "Import:")
	...
	self.controls.charImportFull = new("ButtonControl", { "LEFT", self.controls.charImportHeader, "RIGHT" }, 8, 0, 80, 20,
		"Full Import", function ()
			if self.build.spec:CountAllocNodes() > 0 then
				main:OpenConfirmPopup("Character Import",
					"Full import will overwrite your current passive tree and replace items/skills.", "Import",
					function ()
	...
	self.controls.charImportFullOptions = new("LabelControl", { "LEFT", self.controls.charImportFull, "RIGHT" }, 8, 0,
		200, 16, "(uses below options)")
	self.controls.charImportFull.enabled = function ()
		return self.charImportMode == "SELECTCHAR"
	end
	self.controls.charImportTree = new("ButtonControl", { "LEFT", self.controls.charImportHeader, "LEFT" }, 8, 36, 200,
		20, "Only Passive Tree and Skills", function ()
			if self.build.spec:CountAllocNodes() > 0 then
	...
	self.controls.charImportItems = new("ButtonControl", { "LEFT", self.controls.charImportTree, "LEFT" }, 0, 24, 140, 20,
		"Only Items", function ()
			self:DownloadItems()
		end)
	...
	self.controls.charImportItemsClearItems = new("CheckBoxControl", { "LEFT", self.controls.charImportItems, "RIGHT" },
		120, 0, 18, "Delete equipment:", nil, "Delete all equipped items when importing.", true)
	self.controls.charImportUnusedItemsClearItems = new("CheckBoxControl",
		{ "LEFT", self.controls.charImportItems, "RIGHT" }, 280, 0, 18, "Delete unused items:", nil,
		"Delete all unused items when importing.", false)

	self.controls.charClose = new("ButtonControl", { "TOPLEFT", self.controls.charSelect, "BOTTOMLEFT" }, 0, 136, 60, 20,
		"Close", function ()
			self.charImportMode = "GETACCOUNTNAME"
			self.charImportStatus = "Idle"
		end)
	...
	-- Build import/export
	self.controls.sectionBuild = new("SectionControl", { "TOPLEFT", self.controls.sectionCharImport, "BOTTOMLEFT" }, 0,
		18, 650, 182 + 16, "Build Sharing")
	self.controls.generateCodeLabel = new("LabelControl", { "TOPLEFT", self.controls.sectionBuild, "TOPLEFT" }, 6, 14, 0,
		16, "^7Generate a code to share this build with other Last Epoch Planner users:")
	self.controls.generateCode = new("ButtonControl", { "LEFT", self.controls.generateCodeLabel, "RIGHT" }, 4, 0, 80, 20,
		"Generate", function ()
			self.controls.generateCodeOut:SetText(common.base64.encode(Deflate(self.build:SaveDB("code"))):gsub("+", "-")
				:gsub("/", "_"))
		end)
	self.controls.enablePartyExportBuffs = new("CheckBoxControl", { "LEFT", self.controls.generateCode, "RIGHT" }, 100, 0,
		18, "Export Support", function (state)
		...
		false)
	self.controls.generateCodeOut = new("EditControl", { "TOPLEFT", self.controls.generateCodeLabel, "BOTTOMLEFT" }, 0, 8,
		250, 20, "", "Code", "%Z")
	self.controls.generateCodeOut.enabled = function ()
		return #self.controls.generateCodeOut.buf > 0
	end
	self.controls.generateCodeCopy = new("ButtonControl", { "LEFT", self.controls.generateCodeOut, "RIGHT" }, 8, 0, 60,
		20, "Copy", function ()
	...
	self.controls.exportFrom = new("DropDownControl", { "LEFT", self.controls.generateCodeCopy, "RIGHT" }, 8, 0, 120, 20,
	...
	self.controls.generateCodeByLink = new("ButtonControl", { "LEFT", self.controls.exportFrom, "RIGHT" }, 8, 0, 100, 20,
		"Share", function ()
	...
	self.controls.generateCodeNote = new("LabelControl", { "TOPLEFT", self.controls.generateCodeOut, "BOTTOMLEFT" }, 0, 4,
		0, 14, "^7Note: this code can be very long; you can use 'Share' to shrink it. (Not yet supported)")
	self.controls.importCodeHeader = new("LabelControl", { "TOPLEFT", self.controls.generateCodeNote, "BOTTOMLEFT" }, 0,
		26, 0, 16, "^7To import a build, enter URL or code here:\nNote that you can import from LETools")
	...
	local importSelectedBuild = function ()
		...
		if self.controls.importCodeMode.selIndex == 1 then
			main:OpenConfirmPopup("Build Import",
				colorCodes.WARNING ..
				"Warning:^7 Importing to the current build will erase ALL existing data for this build.", "Import",
				function ()
					self.build:Shutdown()
	...
	self.controls.importCodeIn = new("EditControl", { "TOPLEFT", self.controls.importCodeHeader, "BOTTOMLEFT" }, 0,
		4 + 16, 328, 20, "", nil, nil, nil, importCodeHandle, nil, nil, true)
	...
	self.controls.importCodeMode = new("DropDownControl", { "TOPLEFT", self.controls.importCodeIn, "BOTTOMLEFT" }, 0, 4,
		160, 20, { "Import to this build", "Import to a new build" })
	self.controls.importCodeMode.enabled = function ()
		return self.build.dbFileName and self.importCodeValid
	end
	self.controls.importCodeGo = new("ButtonControl", { "LEFT", self.controls.importCodeMode, "RIGHT" }, 8, 0, 160, 20,
		"Import", function ()
...
function ImportTabClass:DownloadCharacterListOnline()
	...
	launch:DownloadPage("https://www.lastepochtools.com/profile/" .. accountName, function (response, errMsg)
		...
		for i, char in ipairs(charList) do
			char.league = char.cycle == 7 and "Cycle" or "Legacy"
		...
		for _, league in ipairs(leagueList) do
			t_insert(self.controls.charSelectLeague.list, { label = league, league = league, })
		end
		t_insert(self.controls.charSelectLeague.list, { label = "All", })
	...
	for i, char in ipairs(self.lastCharList) do
		if not league or char.league == league then
			...
			t_insert(self.controls.charSelect.list, {
				label = string.format("%s: Level %d %s in %s", char.name or "?", char.level or 0, class,
					char.league or "?"),
				char = char,
			})
		end
...
function ImportTabClass:DownloadFromLETools()
	...
	launch:DownloadPage("https://www.lastepochtools.com/profile/" .. accountName .. "/character/" .. charData.name,
		function (response, errMsg)
			...
			self.charImportStatus = colorCodes.POSITIVE ..
				"Build info successfully downloaded. " ..
				colorCodes.NORMAL ..
				"Last update was on " ..
				lastFetched .. colorCodes.NEGATIVE .. "\n\tManually click on source link to trigger an update"
		end)
...
function ImportTabClass:processItemData(itemData)
	if itemData["containerID"] <= 12 or itemData["containerID"] == 29 or
		...
		local itemBase = self.build.data.itemBases[baseTypeID .. "_" .. subTypeID]
		if itemBase then
			item.baseName = itemBase.name
			item.base = itemBase
			item.implicitMods = {}
...
function ImportTabClass:ReadJsonSaveData(saveFileContent)
	...
	char.league = saveContent["cycle"] == 7 and "Cycle" or "Legacy"
...
function ImportTabClass:ImportPassiveTreeAndJewels(charData)
	self.build.spec:ImportFromNodeList(charData.classId, charData.ascendancy, charData.abilities, charData.hashes,
		charData.skill_overrides, latestTreeVersion)
...
function ImportTabClass:BuildItem(itemData)
	...
	item.rarityType = itemData.rarityType or
		(itemData.rarity == "UNIQUE" and "UNIQUE" or itemData.rarity == "LEGENDARY" and "UNIQUE" or itemData.rarity == "IDOL" and "IDOL" or "BASIC")
		...
		for _, property in pairs(itemData.properties) do
			...
			if property.name == "Energy Shield" or property.name == "Ward" or property.name == "Armour" or property.name == "Evasion Rating" then
				...
				for _, value in ipairs(property.values) do
					item.armourData[property.name:gsub(" Rating", ""):gsub(" ", "")] = (item.armourData[property.name:gsub(" Rating", ""):gsub(" ", "")] or 0) +
						tonumber(value[1])
				end
			end
		end
