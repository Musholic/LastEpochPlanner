...
function SharedItemSetListClass:AddValueTooltip(tooltip, index, sharedItemSet)
	...
	for _, slot in ipairs(self.itemsTab.orderedSlots) do
		if not slot.nodeId then
			...
			if item then
				tooltip:AddLine(16,
					s_format("^7%s: %s%s", self.itemsTab.slots[slotName].label, colorCodes[item.rarity], item.name))
			end
		end
	end
end
...
function SharedItemSetListClass:OnSelDelete(index, sharedItemSet)
	main:OpenConfirmPopup("Delete Item Set",
		"Are you sure you want to delete '" .. (sharedItemSet.title or "Default") .. "' from the shared item set list?",
		"Delete", function ()
