...
local antonyms = { ["increased"] = "reduced", ["reduced"] = "increased", ["more"] = "less", ["less"] = "more", }
...
function itemLib.formatModLine(modLine, dbMode)
	local line = (not dbMode and modLine.range and itemLib.applyRange(modLine.line, modLine.range, modLine.valueScalar, modLine.rounding)) or
		modLine.line
	...
	if modLine.extra then
		...
		colorCode = (modLine.corrupted and colorCodes.CORRUPTED) or (modLine.sealed and colorCodes.SEALED) or
			colorCodes.MAGIC
	end
...
itemLib.wiki = {
	...
	openItem = function (item)
		local name = item.rarityType == "UNIQUE" and item.title or item.baseName
