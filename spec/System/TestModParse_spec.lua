describe("TestModParse", function ()
	...
	it("damage types", function ()
		...
		assert.are.equals(25,
			build.configTab.modList:Sum("BASE", { keywordFlags = bit.bor(KeywordFlag.Fire, KeywordFlag.Spell) },
				"FireDamage"))
	end)
	...
	it("increased damage", function ()
		...
		assert.are.equals(50,
			build.configTab.modList:Sum("INC", { keywordFlags = bit.bor(KeywordFlag.Void, KeywordFlag.Melee) },
				"VoidDamage"))
	end)
	...
	it("effect doubled", function ()
		...
		"+40% Increased fire damage. This effect is doubled if you have 300 or more maximum mana."
		build.configTab:BuildModList()
		runCallback("OnFrame")
	...
	it("elemental cast speed", function ()
		...
		assert.are.equals(10,
			build.configTab.modList:Sum("INC", { keywordFlags = KeywordFlag.Fire, flags = ModFlag.Cast }, "Speed"))
	end)
	...
	it("melee and throwing attack speed", function ()
		...
		assert.are.equals(10,
			build.configTab.modList:Sum("INC", { keywordFlags = ModFlag.Melee, flags = ModFlag.Attack }, "Speed"))
		assert.are.equals(10,
			build.configTab.modList:Sum("INC", { keywordFlags = ModFlag.Throwing, flags = ModFlag.Attack }, "Speed"))
	end)
	...
	it("shred chance", function ()
		...
		assert.are.equals(10,
			build.configTab.modList:Sum("BASE", { flags = ModFlag.Hit },
				"ChanceToTriggerOnHit_Ailment_VoidResistanceShred"))
	end)
	...
	it("melee chance", function ()
		...
		assert.are.equals(0,
			build.configTab.modList:Sum("BASE", { flags = ModFlag.Hit }, "ChanceToTriggerOnHit_Ailment_Ignite"))
		assert.are.equals(10,
			build.configTab.modList:Sum("BASE", { flags = bit.bor(ModFlag.Hit, ModFlag.Melee) },
				"ChanceToTriggerOnHit_Ailment_Ignite"))
	end)
	...
	it("bleed chance", function ()
		...
		assert.are.equals(17,
			build.configTab.modList:Sum("BASE", { flags = ModFlag.Hit }, "ChanceToTriggerOnHit_Ailment_Bleed"))
	end)

	it("decimate cast chance", function ()
		build.configTab.input.customMods =
		"17% Chance to cast Decimate when you directly use a Fire Spell (4 second cooldown)"
		build.configTab:BuildModList()
		runCallback("OnFrame")

		assert.are.equals(17,
			build.configTab.modList:Sum("BASE", { keywordFlags = KeywordFlag.Fire },
				"ChanceToTriggerOnHit_GasparSetSwipe"))
	end)
